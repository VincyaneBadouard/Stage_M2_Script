...
parameters {
  ...
  real theta1a; // theta1a = syviculture parameter croissant part of intermediate HP
  real theta1b; // theta1b = syviculture parameter descending part 
...
model {
  DivTaxo_Shann ~ lognormal(log(theta0 + theta1a*Sylv + theta1b*Sylv .* Sylv + theta2*Fire + theta3*Ced + theta4*Soil), sigma); // Likelihood
}
