...
#To standardize variables to put them at the same scale and to keep theta0 as mean of the response variable :
...
## Data preparation
```{r}
N = dim(standardcovariantsdata) [1] #rows (observations (plots)) number
...
Soil = standardcovariantsdata$prop_hydrosoil #we take only the hydromorphic soil

## Interaction effect
Interac = covariantsdata$prop_ba_removed * covariantsdata$prop_fire # fire & sylv multiplication no standardized
Interac_std = (as.vector(scale(Interac))) # fire & sylv standardized multiplication
           
# Predictions 
N_pred = 100 # values number that we want to predict

## Create 100 no-standardized values of each predictor
# with max & min of initial no-standardized variables   
...
# To standardize the sampled predictors :
# standardized with INITIAL variable's mean and scale to not change relation with the theta model's parameter.
Silv_pred = (Silv_seq - mean(covariantsdata$prop_ba_removed))/sd(covariantsdata$prop_ba_removed) 
...
```{r, results = 'hide'}
# Taxonomic diversity
# Data only for IDH models (without prediction)
data_Taxo <- list( 
  N = N, # Observations number (plots number)
  DivTaxo_Shann = DivTaxo_Shann, # Response variable
  #Effects variables (standardized):
  Sylv = Silv,
  Fire = Fire,
  Ced = Ced,
  Soil = Soil 
...
data_Phylo <- list( 
  N = N, # Observations number (plots number)
  DivPhylo_Shann = DivPhylo_Shann,  # Response variable
  #Effects variables (standardized):
  Sylv = Silv,
  Fire = Fire,
  Ced = Ced,
...
# Fctnal diversity
data_Fctnal <- list( # Data only for IDH models (without prediction)
  N = N,# Observations number (plots number)
  DivFctnal_Shann = DivFctnal_Shann,  # Response variable
  #Effects variables (standardized):
  Sylv = Silv,
  Fire = Fire,
  Ced = Ced,
...
plot(DivTaxo_ShannM2_IDH_Ced, pars = c("theta0","theta1","theta2","theta3a", "theta3b","theta4"))
...
plot(DivTaxo_ShannM3_IDH_Sylv, pars = c("theta0","theta1a","theta1b","theta2","theta3", "theta4"))
...
plot(DivTaxo_ShannM4_IDH_Fire, pars = c("theta0","theta1","theta2a","theta2b","theta3", "theta4"))
...
plot(DivTaxo_ShannM5_interac_fire_sylv, pars = paste0("theta", 0:5))
mcmc_intervals(DivTaxo_ShannM5_interac_fire_sylv, regex_pars = "theta3", prob_outer = 0.9) #only Cedrela's effect
```
...
plot(DivPhylo_ShannM2_IDH_Ced, pars = c("theta0","theta1","theta2","theta3a", "theta3b","theta4"))
...
plot(DivPhylo_ShannM3_IDH_Sylv, pars = c("theta0","theta1a","theta1b","theta2","theta3", "theta4"))

```
Silviculture is not an "intermediate disturbance" : theta1a is negativ and theta1b is positiv.
Cedrela : negativ effect
Fire : positiv effect
Soil : positiv effect
...
plot(DivPhylo_ShannM4_IDH_Fire, pars = c("theta0","theta1","theta2a","theta2b","theta3", "theta4"))
...
# png(file = "C:/Users/Utilisateur/Desktop/Stage_M2_CI/Figures/Prediction_plots_Phylo.png", width=738, height=512)
...
plot(DivFctnal_ShannM2_IDH_Ced, pars = c("theta0","theta1","theta2","theta3a", "theta3b","theta4"))
...
plot(DivFctnal_ShannM3_IDH_Sylv, pars = c("theta0","theta1a","theta1b","theta2","theta3", "theta4")) 

```
Sylviculture is not an "intermediate disturbance" : theta1a is negativ and theta1b is positiv (or no effect).
Cedrela : negativ effect
Fire : positiv effect
Soil : no effect
...
plot(DivFctnal_ShannM4_IDH_Fire, pars = c("theta0","theta1","theta2a","theta2b","theta3", "theta4"))

```
Fire is not an "intermediate disturbance" : theta2b is positiv (or no effect).
Silv : negativ effect
Cedrela : negativ effect
Soil : no effect
...
# png(file = "C:/Users/Utilisateur/Desktop/Stage_M2_CI/Figures/Prediction_plots_Fctnal.png", width=738, height=512)
